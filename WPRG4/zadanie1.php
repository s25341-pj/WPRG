...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($error)) {
            $file = "rezerwacje.csv";
            $header = array("Ilość osób", "Imię", "Nazwisko", "Adres", "Numer karty kredytowej", "E-mail", "Data początku rezerwacji", "Data końca rezerwacji", "Dostawka dla dziecka", "Udogodnienia");
            $data = array();
            $data[] = $_POST["numbers"] . ";" . $_POST["name"] . ";" . $_POST["surname"] . ";" . $_POST["address"] . ";" . $_POST["numCard"] . ";" . $_POST["email"] . ";" . $_POST["start_date"] . ";" . $_POST["end_date"] . ";" . (isset($_POST["child_bed"]) ? "Tak" : "Nie") . ";" . implode(",", $_POST["amenities"]);
            $fp = fopen($file, 'a');
            if ($fp) {
                if (filesize($file) == 0) {
                    fputcsv($fp, $header, ";");
